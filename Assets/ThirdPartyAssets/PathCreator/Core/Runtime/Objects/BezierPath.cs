...
namespace PathCreation
{
    ...
    public class BezierPath
    {
        ...
        public ClosePointData GetClosestPointOnPath(Vector3 pos)
        {
            ...
            for (int i = 0; i <= NumSegments; i++)
            {
                ...
                var closestPoint = CubicBezierUtility.ClosestPointOnCubicBezier(segment, pos);
                var distance = Vector3.Distance(pos, closestPoint);

                if (!(distance < pointData.distance)) continue;
                pointData.distance = distance;
                pointData.point = closestPoint;
            }
            
            return pointData;
        }
        public ClosePointData GetClosestPointOnPath(Vector3 pos, float timeStep, int iterations)
        {
            ClosePointData pointData = new ClosePointData();
            pointData.distance = float.MaxValue;
